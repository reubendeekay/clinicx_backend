...
def create_payment(
    ...
    payment_query = db.query(models.Payment).filter(
        models.Payment.appointment_id == payment.appointment_id
    )
    if payment_query.first() is not None:
        raise HTTPException(
            status_code=HTTPStatus.CONFLICT, detail="Payment already exists"
        )
...
@router.put(
    "/{payment_id}", status_code=HTTPStatus.OK, response_model=schemas.PaymentOut
)
def update_payment(
    payment_id: int,
    updated_payment: schemas.PaymentBase,
    db: SessionLocal = Depends(get_db),
    current_user: schemas.User = Depends(oauth2.get_current_user),
):
    payment = (
        ...
        .update(updated_payment.dict(), synchronize_session=False)
    )
    db.commit()
...
def get_payment_count_by_patient(
    patient_id: int,
    db: SessionLocal = Depends(get_db),
    current_user: schemas.User = Depends(oauth2.get_current_user),
):
    return (
        ...
        .filter(models.Payment.appointment.patient_id == patient_id)
        .scalar()
    )
...
def get_payment_count_by_doctor(
    doctor_id: int,
    db: SessionLocal = Depends(get_db),
    current_user: schemas.User = Depends(oauth2.get_current_user),
):
    return (
        db.query(func.count(models.Payment.id))
        .filter(models.Payment.appointment.doctor_id == doctor_id)
        .scalar()
    )
...
def get_payment_count_by_branch(
    branch_id: int,
    db: SessionLocal = Depends(get_db),
    current_user: schemas.User = Depends(oauth2.get_current_user),
):
    return (
        db.query(func.count(models.Payment.id))
        .filter(models.Payment.appointment.branch_id == branch_id)
        .scalar()
    )
